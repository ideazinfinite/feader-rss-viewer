...
var cipherTxt="theLAZYfoxDIDjumpOVERfeNCE",cipherKey;
...
/* <![CDATA[ */
var loadTime=(new Date()).getTime();var key;var prng;function setKey(cipherKey)
{{var s=cipherKey;var hexDigits="0123456789abcdefABCDEF";
var hs="",i,bogus=false;for(i=0;i<s.length;i++){var c=s.charAt(i);
if(hexDigits.indexOf(c)>=0){hs+=c;}else{bogus=true;}}if(bogus){alert("Error: Improper character(s) in hexadecimal key." + cipherKey);}
if(hs.length>(keySizeInBits/4)){alert("Warning: hexadecimal key exceeds "+(keySizeInBits/4)+" digit maximum; truncated.");
cipherKey=hs=hs.slice(0,64);}else{while(hs.length<(keySizeInBits/4)
){hs+="0";}}key=hexToByteArray(hs);}}function Generate_key(){var i,j,k="";
var i,j,k="";addEntropyTime();var seed=keyFromEntropy();
var prng=new AESprng(seed);var hexDigits="0123456789ABCDEF";
for(i=0;i<64;i++){k+=hexDigits.charAt(prng.nextInt(15));}
cipherKey=k;delete prng; return cipherKey}
function Encrypt_text(cipherKey,rilPass){var v,i;
var prefix="#####  Encrypted: decrypt with http://www.fourmilab.ch/javascrypt/\n",suffix="#####  End encrypted message\n";if(cipherKey.length==0)
{alert("Please specify a key with which to encrypt the message.");return;}if(rilPass.length==0){alert("No plain text to encrypt!  Please enter or paste plain text in the field above.");
return;}encRilPass="";setKey(cipherKey);addEntropyTime();
prng=new AESprng(keyFromEntropy());
var plaintext=encode_utf8(rilPass);md5_init();for(i=0;
...
v=armour_base64(ct);
encRilPass=prefix+v+suffix;delete prng; return encRilPass;}
...
function Decrypt_text(cipherKey,encRilPass){if(cipherKey.length==0){alert("Please specify a key with which to decrypt the message.");
return;}if(encRilPass.length==0){alert("No cipher text to decrypt!  Please enter or paste cipher text in the field above.");return;}
//rilPass="";
setKey(cipherKey);var ct=new Array(),kt;
kt=determineArmourType(encRilPass);if(kt==0)
{ct=disarm_codegroup(encRilPass);}else if(kt==1)
{ct=disarm_hex(encRilPass);}else if(kt==2)
{ct=disarm_base64(encRilPass);}var result=rijndaelDecrypt(ct,key,
"CBC");var header=result.slice(0,20);result=result.slice(20);
...
break;}}rilPass=decode_utf8(plaintext); return rilPass;}
/* ]]> */